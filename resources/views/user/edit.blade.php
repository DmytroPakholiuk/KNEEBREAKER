...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-12">
                    <h1>Users \ Edit user {{ $user->name }}</h1>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="row justify-content-center">
            <div class="col-12 card py-3 px-3">
                <form action="{{ route('users.update', $user) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <x-adminlte-input name="name" label="Name" placeholder="Enter name" value="{{ old('name', $user->name) }}"
                                          fgroup-class="col-12"/>
                    </div>

                    <div class="row">
                        <x-adminlte-input type="email" name="email" label="Email" placeholder="Enter email" value="{{ old('email', $user->email) }}"
                                          fgroup-class="col-12"/>
                    </div>
